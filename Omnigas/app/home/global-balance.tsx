    ...
    useEffect(() => {
        const calculateTotalBalance = () => {
            const total = Object.keys(tokens)
                .filter(key => chain === undefined || chains[tokens[key].network].viem === chain)
                .map(key => parseFloat(tokens[key].balance!) * parseFloat(tokens[key].rate!))
    ...
    return ( 
        <div className='h-[25vh] w-full flex items-center justify-center'>
            <div className="flex items-center">
                  <h1 className="text-6xl font-semibold tracking-wide">${totalBalanceUSD.split(".")[0]}</h1>
                  <h1 className="text-lg items-start pb-6 text-card font-semibold tracking-wide">{"." + totalBalanceUSD.split(".")[1]}</h1>
            </div>
        </div>
    );
}
